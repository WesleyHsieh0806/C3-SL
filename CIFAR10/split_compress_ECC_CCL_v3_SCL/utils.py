...
def SC_Loss(z, recover_z):
    '''
    * z, recoverz: (Batch, nof_feature)
    * MSE between SC matrix
    '''
    # Normalize along the batch dimension
    z = (z - z.mean(1, keepdim=True))
    norm_z = z / torch.norm(z, dim=1, keepdim=True)  # (B, dim)
    SC_norm_z = torch.mm(norm_z, norm_z.T)

    # Normalize along the batch dimension
    recover_z = (recover_z - recover_z.mean(1, keepdim=True))
    norm_recover_z = recover_z / \
        torch.norm(recover_z, dim=1, keepdim=True)  # (B, dim)
    SC_norm_recover_z = torch.mm(norm_recover_z, norm_recover_z.T)
    return torch.mean((SC_norm_z-SC_norm_recover_z)**2)
