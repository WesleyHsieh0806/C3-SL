...
class SplitResNet50(nn.Module):
    ...
    def forward(self, image):
        ...
        * Reconstruction loss is computed between z and recover_z
        ...
        shape = z.shape
        z = z.flatten(start_dim=1)
        norm_z, STD, MEAN = normalize_for_circular(z)  # normalize

        # ECC Encryption
        compress_V, recover_z = self.ecc(norm_z)
        de_recover_z = recover_z * (STD*sqrt(recover_z.shape[-1]))
        de_recover_z = de_recover_z + MEAN
        self.front = [
            z, F.relu(de_recover_z, inplace=True), compress_V, norm_z]
        ...
        **********
        ...
        # ECC Decryption
        remote_recover_z = self.ecc.decrypt(self.remote[0])
        de_remote_recover_z = remote_recover_z * \
            (STD*sqrt(remote_recover_z.shape[-1]))
        de_remote_recover_z = de_remote_recover_z + MEAN
        de_remote_recover_z = F.relu(de_remote_recover_z, inplace=True)

        return self.models[1](de_remote_recover_z.reshape(shape))

    def backward(self, L_CE, L_rec, Lambda, L_SC, Lambda2):
        ''' When we call L_CE.backward(), it only backwards for the last half layers
        * So here we will manually compute the gradient for the front-half convblocks
        '''
        # Compute the total loss first
        L = L_CE + Lambda * L_rec + L_SC * Lambda2
        L.backward(retain_graph=True)
