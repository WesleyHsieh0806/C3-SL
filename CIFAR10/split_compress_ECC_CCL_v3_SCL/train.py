...
from utils import BT_Loss, SC_Loss
...
parser.add_argument("--epoch", required=False, type=int,
                    default=30, help="The Number of Epoch")
parser.add_argument("--Lambda", required=False, type=float,
                    default=0.0001, help="The Lambda for extra Loss term")
parser.add_argument("--beta", required=False, type=float,
                    default=0.0001, help="The beta for off-diagonal terms")
parser.add_argument("--Lambda2", required=False, type=float,
                    default=0.0001, help="The Lambda2 for extra Loss term")
...
Lambdas2 = [args.Lambda2 * (0.99**i) for i in range(args.epoch+1)]
...
for epoch in range(start_epoch, num_epoch+1):
    ...
    for i, (train_x, train_y) in enumerate(Train_Loader, 1):
        ...
        batch_L_BT = BT_Loss(model.front[3], args.beta)  # norm_z
        batch_L_SC = SC_Loss(model.front[0], model.front[1])
        ...
        batch_L_grad_rec = model.backward(
            batch_L_CE, batch_L_BT, Lambda, batch_L_SC, Lambda2)
        ...
        train_SC_loss += len(train_x) * (batch_L_SC).item()
    ...
    train_SC_loss /= Train_Dataset.__len__()
    ...
    # Output the result
    print("Epoch [{}/{}] Time:{:.3f} secs Train_acc:{:.4f} train_CE_loss:{:.4f} train_BT_loss:{:.4f} train_SC_loss:{:.4f}".format(epoch, num_epoch, time.time()-epoch_start_time,
                                                                                                                                  train_acc, train_CE_loss, train_BT_loss, train_SC_loss))
...
with open(os.path.join(saved_path, "train_SC_loss.csv"), "w") as f:
    for i in range(len(train_SC_loss_list)-1):
        f.write(str(train_SC_loss_list[i])+",")
    f.write(str(train_SC_loss_list[-1]))
