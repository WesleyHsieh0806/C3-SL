...
learning_rate = 1e-4
Lambdas = [0.0001 for i in range(args.epoch+1)]
...
for epoch in range(1, num_epoch+1):
    ...
    for i, (train_x, train_y) in enumerate(Train_Loader, 1):
        ...
        model.backward(batch_L_CE, batch_L_rec, Lambda)
...
with open(os.path.join(
        os.path.dirname(__file__), "log", "train_accuracy_{}.csv".format(args.batch)), "w") as f:
    for i in range(len(train_acc_list)-1):
        f.write(str(train_acc_list[i])+",")
    f.write(str(train_acc_list[-1]))
with open(os.path.join(
        os.path.dirname(__file__), "log", "train_CE_loss_{}.csv".format(args.batch)), "w") as f:
    for i in range(len(train_CE_loss_list)-1):
        f.write(str(train_CE_loss_list[i])+",")
    f.write(str(train_CE_loss_list[-1]))

with open(os.path.join(
        os.path.dirname(__file__), "log", "train_rec_loss_{}.csv".format(args.batch)), "w") as f:
...
with open(os.path.join(
        os.path.dirname(__file__), "log", "test_accuracy_{}.csv".format(args.batch)), "w") as f:
    for i in range(len(test_acc_list)-1):
        f.write(str(test_acc_list[i])+",")
    f.write(str(test_acc_list[-1]))

with open(os.path.join(
        os.path.dirname(__file__), "log", "test_CE_loss_{}.csv".format(args.batch)), "w") as f:
    for i in range(len(test_CE_loss_list)-1):
        f.write(str(test_CE_loss_list[i])+",")
    f.write(str(test_CE_loss_list[-1]))
with open(os.path.join(
        os.path.dirname(__file__), "log", "test_rec_loss_{}.csv".format(args.batch)), "w") as f:
