...
for epoch in range(start_epoch, num_epoch+1):
    ...
    for i, (train_x, train_y) in enumerate(Train_Loader, 1):
        ...
        batch_L_BT = BT_Loss(model.front[0], args.beta)
    ...
    # Output the result
    print("Epoch [{}/{}] Time:{:.3f} secs Train_acc:{:.4f} train_CE_loss:{:.4f} train_BT_loss:{:.4f}".format(epoch, num_epoch, time.time()-epoch_start_time,
                                                                                                             train_acc, train_CE_loss, train_BT_loss))
